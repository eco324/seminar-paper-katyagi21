...
hh_2005 <- data_2005 %>% 
  select(hh_id, assist, reason, sex, age,
         relation, literacy, register, school_level, grade, repeated, depto, urb_rur, income, educ, job, tasks, pov_line, extr_pov) %>% 
  mutate(year = 2005,
...
hh_head <- subset(total_data, relation == 1, select = c("hh_id", "sex", "age", "literacy", "dept", "urb_rur", "income", "year", "educ", "job", "tasks", "pov_line", "extr_pov"))
...
children <- subset(total_data, age < 18 & age > 4, select = c("hh_id", "age", "sex", "relation", "literacy", "dept", "urb_rur", "income", "assist", "reason", "register", "school_level", "grade", "repeated", "floods", "droughts", "year", "job", "tasks"))
...
hh_head_2 <- hh_head %>%
  select(sex, age, literacy, educ, job, tasks, pov_line, extr_pov, hh_id) %>%
  rename (female_hhhead = sex,
          hhhead_age = age,
...
total_subset %>%
  mutate(ln_income = log(income),
         ...
         female = sex-1,
         no_assist = assist-1,
         not_registered = register-1)
#trying to make the assist, register, sex, and literacy variables into 0 1 binary variables, with assist and register = 1, sex = 1 if female, and literacy = 1 if literate. The current coding from the original data is all in 1 2 format, with the opposites. Trying to change it by subtracting doesn't seem to be working.

=======
>>>>>>> f280b9b927bae959861efd5863a0d6e54b157b58
...
#model_assist <- lm(assist ~ floods + droughts + age + sex + income + hhhead_educ + female_hhhead + year, data = reg_data)
#summary(model_assist)
model_register <- lm(register ~ floods + droughts + age + sex + income + hhhead_educ + female_hhhead + year, data = reg_data)
...
rural <- subset(reg_data, urb_rur == 2, select = c("hh_id", "id", "age", "sex", "literacy", "dept", "urb_rur", "income", "assist", "register", "floods", "droughts", "year", "job", "tasks", "num_children", "female_hhhead", "hhhead_literacy", "hhhead_educ", "year_factor"))

model_assist_r <- lm(assist ~ floods + droughts + age + sex + income + hhhead_educ + female_hhhead + urb_rur + year, data = reg_data)
summary(model_assist_r)
model_register_r <- lm(register ~ floods + droughts + age + sex + income + hhhead_educ + female_hhhead + urb_rur + year, data = reg_data)
