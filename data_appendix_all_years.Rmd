...
hh_2005 <- data_2005 %>% 
  select(hh_id, assist, reason, sex, age,
         relation, literacy, register, school_level, grade, repeated, depto, urb_rur, income) %>% 
  mutate(year = 2005,
...
all_years <- hh_2005 %>% 
  ...
  #bind_rows(hh_2005, hh_2006, hh_2007, hh_2011, hh_2012)
  #rbind(hh_2005, hh_2006) 
...
hh_head <- subset(total_data, relation == 1, select = c("hh_id", "sex", "age", "literacy", "dept", "urb_rur", "income", "year"))
...
children <- subset(total_data, age < 18 & age > 4, select = c("hh_id", "age", "sex", "relation", "literacy", "dept", "urb_rur", "income", "assist", "reason", "register", "school_level", "grade", "repeated", "floods", "droughts", "year"))
                
```

```{r combining household head characteristics with children dataset}
hh_head_2 <- hh_head %>%
  select(sex, age, literacy, hh_id) %>%
  rename (female_parent = sex,
          parent_age = age,
          parent_literacy = literacy)
